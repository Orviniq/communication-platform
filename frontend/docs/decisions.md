...
| ADR-073 | Accepted | The first measurement of a phase-4 build against the live deployment: the send-response defect is confirmed gone, the send failure a user still sees is upstream of the send, and the instrument that should name it cannot resolve past `api` (2026-09-09) | ADR-071 and ADR-072 both close with **Not verified: no run against the live server** — every check behind them is a mock adapter or a unit test read against `backend/openapi.json`. Beta build 25 is the first artifact carrying them onto the A56, which holds a registered device and a signed-in account, and it settles three things those tests could not. **The parser defect is gone, and one counter proves it.** Before ADR-071 the send response was required to hold exactly two keys while `SendOut` has required three since the route existed, so every `POST /api/v1/envelopes` threw `MalformedApiBody`, arrived as `SecurityFailure(malformedServerResponse)`, and — not being a permanent send failure — returned to `retryWait` under a backoff capped at fifteen minutes with no attempt ceiling above it. The server had answered `202` and written the rows, so the same batch went again on every cycle: a message delivered, shown as unsent, and delivered again on each retry. The export now reads `malformed_response=0` over a session that both sent and received, and that counter could not be zero while the defect stood. **The failure a user sees is a different one, and it survives phase 4.** Messages sent at 10:50 sit refused with `Retry as a new encrypted send` while a message received at 10:58 landed in the same conversation, so the socket, the drain, the session and the pairwise state with that peer are all working. The export reads `session=fullScope`, `delivery_session=running`, `queue_gap_detected=no`, `database_schema=19`, `pending_outbound=0`, `pending_inbound=0` and `quarantined_input=0` beside `succeeded=10-99`, `backend_rejected=10-99`, `transport_failed=0`, `cancelled=0` and `size_rejected=0`. Nothing is queued, nothing is quarantined and no transport attempt failed, so the work is not waiting and the network is not the cause — [[a56-vpn-breaks-backend-connection]] is ruled out by `transport_failed=0` rather than assumed away. `most_failed_operation=api` puts the refusals on the generic bucket rather than on `syncSend`, and `_deriveTransportState` reaches `permanentlyFailed` for an event with no outbox rows whose preparation row is not `owed` — so the fault is in **preparation**, before any envelope is queued, on one of the reads a send owes before it can seal: the peer identity, the peer device list, the device log, or the bundle claim. **The strict response validation stays, and the reason is not the one the earlier report gave.** That report recommended dropping the key count everywhere, on the ground that the server calls an added response field additive. The published document agrees with the premise and not the conclusion: 21 of the `*In` schemas carry `additionalProperties: false` and **none of the 34 `*Out` schemas do**, so the contract is deliberately asymmetric — closed inbound, open outbound — and a client counting response keys does contradict it. It is still the safer of the two failures here. A tolerant reader that had not been taught `full_devices` would have ignored it, and `full_devices` means *this message was not stored*; the count turned that into a loud total outage instead of a silent report of delivery that did not happen, which is the trade ADR-052 and `backend/SECURITY.md` both settle the same way. The count is therefore kept until a client-side gate proves every parser covers the required fields of `backend/openapi.json`. **That gate, not tolerance, is what makes the count removable**, and it does not exist. **The instrument cannot name the survivor, and that is the finding that blocks the fix.** `NetworkOperation.api` collapses nine routes into one value and the report carries no status code, both deliberately — ADR-057 made redaction structural, so the export holds an enumeration and a bounded integer and can hold no identifier. `most_failed_operation=api` is therefore the finest answer it can give, and it cannot say which route the deployment refused or with which code. Naming it needs the temporary `print` in a release build that ADR-064 used, or the deployment's own access log; neither is done here, because this record changes no code. Adds no dependency, opens no production gate, changes no wire format, and touches no `backend/` file. **Not done here**: the preparation fault is described and not fixed. **Not verified**: which route answers the refusal, and with which code. |
