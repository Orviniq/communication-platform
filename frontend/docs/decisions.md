...
| ADR-071 | Accepted | The published limits replace the wire constants at every call site, the send path reads `full_devices`, and the disclosure states the deployment's retention window rather than a time it cannot name — taking the disclosure to revision 8 (2026-09-09) | ADR-070 wrote the `attachment_ttl_days` copy down as a knowing one and said a later phase would replace it. This is that phase, and it found the same shape in ten more places. **The constants were a second statement of a limit.** `ApiContractLimits` held `send_batch_max`, `ack_max`, `claim_max`, `drain_page_max`, the envelope buckets and the attachment buckets as numbers copied out of the backend documents, and two Drift stores held the envelope bucket set a second and third time; an operator who moved any of them had clients that disagreed with the deployment they were talking to, and the two copies inside this client could disagree with *each other* — a page the transport accepted and the store refused would be an envelope lost between them. They are read from `GET /api/v1/config` now, through a follower that answers synchronously, because the readers cannot await: a socket frame is validated as it arrives and a response inside a `decode` callback. The value moves without the socket or the delivery cycle being rebuilt around it. **A ceiling is held against, not enforced by, the caller.** A page size and an acknowledgement batch are cut down to `drain_page_max` and `ack_max` by the adapters that can see them, because a refused page is envelopes left undrained over a number the application layer has no business knowing — and the engine therefore imports nothing from the configuration feature, which the architecture boundary forbids in any case. A body past `send_batch_max` or `claim_max` stays a caller fault: the route refuses it whole, and a refused claim consumes nobody's prekeys. **`full_devices` was never read.** `POST /api/v1/envelopes` has answered with it beside `stale_devices` since the route existed, and this client read only the second, so every device whose mailbox had reached `MAILBOX_MAX_BYTES` was counted as written to and a message that was never stored was reported delivered — on a direct message as much as a group fan-out. The two answers are opposites (`backend/CLIENT_CONTRACT.md` §F): a stale device is gone and its sessions go with it, a full device is live and its owner has simply not collected their post, so its row waits out a backoff and comes back and nothing about it is torn down. `accepted` is checked against the batch less both, and an item larger than `mailbox_max_bytes` is refused once rather than reported full forever. **Disclosure revision 8.** `ENVELOPE_TTL_DAYS` is the reason the configuration route exists: it has no other observable, because nothing refuses and so no error teaches it, and revision 5 could therefore only say "a time set by whoever runs the server". The point now states the deployment's own window — and *only* its own. Until the route has answered, this build's default is a guess, and a guess in a mandatory statement is the class of claim ADR-052 was written to remove, so the wording without a number is kept for exactly that case rather than deleted. That distinction needs provenance the wire cannot carry, so `ServerConfig` gains one field that is not a limit: whether these numbers came from the deployment. ADR-052's mechanism did the rest unaided — the edit forced `since: 8`, which forced the revision, and `DisclosureChangeGate` re-presents once to a revision-7 reader with that single point badged. **Re-delivering the written handover disclosure is release-blocking and has not happened**, now at revision 8, superseding the outstanding revision-7 obligation rather than adding to it. Adds no dependency, opens no production gate, changes no wire format, and touches no `backend/` file. **Not done here**: `attachment_daily_bytes` states no remaining allowance, because this build composes no attachment picker and no upload path, so there is no send for it to precede. |
| ADR-072 | Accepted | The client's error vocabulary becomes the one the surface answers: four codes no route returns are deleted, seven the routes do return are added, and the two pairs that share a status are kept apart by `code` at every reader (2026-09-09) | ADR-071 read the published limits and left the published *errors* alone. `BackendFailureCode` still held `bad_request`, `token_not_valid`, `device_scope_required` and `keypackage_limit`. The first two were renamed at the rebuild, the third is returned by no route on a surface where every requirement supplies a device-bound token, and the fourth belonged to a KeyPackage route that left with MLS. **A value nothing answers is worse than a missing one.** It reads like coverage: a branch on `bad_request` looks like the malformed-body case handled, and every malformed body had been arriving as `invalid_request` and falling through to a catch-all instead. Seven codes the surface does answer had no value at all — `throttled`, `unavailable`, `payload_too_large`, `devicelog_limit`, `storage_full`, `method_not_allowed` and `server_error` — so each of them was `unknown`, or was placed by its status and nothing said so. Every value is now the lowerCamelCase of the code it maps, one for one, and a test walks the table in both directions: a code with no value fails it, and so does a value no code reaches. **Two pairs share a status, and a status is therefore not a branch.** `429 throttled` says this client asked too often and names the wait in `Retry-After`; `503 unavailable` says the server is saturated or a store it needs is gone, and publishes no wait — so the header is carried on the one code that publishes one and dropped everywhere else, because a caller reading "come back in 17 seconds" off an outage would be quoting a number the server never gave. `413 quota_exceeded` is this account's own allowance for this UTC day, spent, with nothing stored and nothing charged: a retry before 00:00 UTC answers identically, so the attachment is held until the day turns. `413 payload_too_large` shares that status and no waiting fixes it. `503 storage_full` is the operator's disk, is not the account's fault, and clears when space is freed rather than when the day does. The attachment upload had been reading `413` as `quota_exceeded` outright; it reads the envelope now, because both pairs land on that one route, and the screen owes each half different words. **Naming the 5xx codes changed nothing in the durable queue, on purpose.** They had arrived as `unknown`, which the send classifier treats as transient, and they stay transient now that they have names: an outage, a disk the operator has to free and an internal failure are facts about the server rather than about the bytes in hand, and retiring a message on one would tell somebody their message failed because a server hiccuped once. `payload_too_large` and `method_not_allowed` join the permanent set instead, because the same body and the same method answer the same way forever. `voice_unconfigured` is kept exactly as it was: a third meaning at `503`, not a backoff, read as "this deployment does not do voice". Adds no dependency, opens no production gate, changes no wire format, and touches no `backend/` file. **Not done here**: no localized string moves, because no screen renders any of the seven new codes yet — the vocabulary is correct before the copy that will read it exists. **Not verified**: no run against the live server; every check is a mock-adapter or unit test against `backend/openapi.json` and `backend/core/API.md`. |
...
[`docs/architecture/decisions/0001-pairwise-double-ratchet-group-fan-out.md`](../../docs/architecture/decisions/0001-pairwise-double-ratchet-group-fan-out.md)
(accepted 2026-09-03). Group protocol, storage and UI decision. **Closes** ADR-026,
ADR-036, ADR-037, ADR-039, ADR-040, ADR-041, ADR-044, ADR-055 and ADR-056. It adds no
ciphersuite, primitive, key or server route: the signed group control events, the
`CPGSV001` payloads and schema 22 it records were introduced earlier on the same branch.

### The question

> Piece 19 was recorded as blocked on five prerequisites nobody in this project could meet.
> The server has since deleted MLS. Is piece 19 still blocked, and what does a group owe the
> person using it now?

### What the server decided

Server ADR-0001 moves groups off MLS. A group is a set of pairwise PQXDH plus Double Ratchet
sessions fanned out over ordinary envelopes (`backend/CLIENT_CONTRACT.md` §F), and the
server holds no group object, roster, epoch or group key. The three key-package routes —
`PUT /api/v1/me/devices/{device_id}/keypackages`,
`GET /api/v1/me/devices/{device_id}/keypackages/count` and
`POST /api/v1/users/{user_id}/keypackages/claim` — answer `404`. `backend/openapi.json`,
which CI gates in both directions, holds 28 paths, and none of them accepts, stores or
serves a key package, a Welcome, a commit or any other MLS artefact. `KEYPACKAGE_BUCKETS`
is gone from the server, so nothing pads to 4,096 or 16,384 bytes for a key package. That
decision is made and correct, and this record does not reopen it.

### Decision

1. **Piece 19 is cancelled, not blocked.** Its five external prerequisites — the primitive
   mapping, the MLS suite identifier, maintained provider support, upstream
   interoperability vectors and a retained independent reviewer — do not matter now,
   because no server serves MLS: satisfying every one of them would leave nothing to talk
   to. They are not audited again.
2. **These decisions are closed by it.**

   | ADR | What it decided | Why it is closed |
   |---|---|---|
   | ADR-026 | The IETF PQ MLS candidate suite, with no locally assigned production identifier | No MLS suite is used at all |
   | ADR-036 | A real, isolated closed-beta PQ MLS track behind a closed production gate | The track, its gate and its core are deleted |
   | ADR-037 | Closed-beta MLS transport v3, carrying the complete control transcript for Welcome and re-add | There is no Welcome; a new member is sent the signed transcript as a `CPGSV001` payload over pairwise envelopes |
   | ADR-039 | Leave as an announcement followed by the owner's `Remove` commit | Leaving is one signed control event that removes the leaver; no commit exists |
   | ADR-040 | The closed-beta hybrid KEM stays as `mls-rs` provides it | No MLS KEM is used |
   | ADR-041 | The ordering key on which same-revision MLS control forks converge | A fork is quarantined, and the client chooses no branch |
   | ADR-044 | One Private Experimental artifact with declared maturity tiers, groups among them | The experimental group tier, its permit and the `beta` flavor that carried it are deleted |
   | ADR-055 | Group messaging withheld behind an evidence ledger for the packaged core | The core the ledger waited on, and the ledger, are deleted |
   | ADR-056 | The measured ABI opens, and the evidence gate becomes per-ABI | The measured core and the per-ABI permit are deleted |

3. **The group screens open on every build.** `groupFeatureAvailabilityProvider` and every
   check of it are gone, and so is the gate each group screen used to return first.
4. **The screen states what a group message costs.** A group message is one encrypted copy
   for each device of each member. Group details and Group Info say so, with a figure a
   reader can check: a group of 50 people with three devices each is about 150 copies of
   one message. Server ADR-0001 caps a group at 50 members and a member at 10 devices, so
   the ceiling is 500 copies plus the sender's own devices.
5. **A send is not shown as complete before its fan-out ends.** A group message reads as
   sent only when none of its copies is still owed. Until then it shows how many copies
   the server has accepted out of the copies owed to devices still in the set. A device the
   server reports in `stale_devices` leaves that count; a device in `full_devices` stays in
   it until its copy is accepted. A failed group message offers a retry of the same
   message, held to the same membership rule as a send.

### Why the count reads like that

The transport state a direct message already derives treats a partly accepted fan-out as
accepted by the relay. For a group that is the first of 150 copies drawing the accepted
mark, so the group timeline maps it to sending instead, and only a state with nothing owed
maps to sent. The count is read from `outbox_operations` for the group's messages whose
status is still queued, sending or partially accepted, so it costs what is in flight rather
than what is in the history, and a timeline whose count has not loaded still says sending.

### What was checked

- `test/features/groups/application/group_fanout_delivery_test.dart`, against the real group
  store, audience resolver and durable outbox: one copy for each live device; a
  `stale_devices` answer removes the device, and the next message is sealed without it; a
  `full_devices` answer keeps the device and its session and sends the same bytes again when
  the backoff ends; a member removed between writing and sealing a message gets no copy.
- `test/features/pairwise/pairwise_post_quantum_refusal_test.dart`: a claimed bundle with no
  PQ material is refused by the claim, is never handed to the core, and leaves a fan-out
  with no session and no copy.
- `test/features/synchronization/durable_sync_engine_test.dart`: a full device is sent its
  item again once the backoff ends, and not before.
- A control event with a bad signature is refused by the inbound coordinator, by the
  control-crypto adapter, and in the Rust core's own tests.
- `test/features/groups/infrastructure/drift_group_repository_test.dart` and
  `test/widget/group_pages_test.dart`: the delivery mapping, the copy count, the retry and
  the cost statement.

### What this does not do

It runs nothing on a physical device or against a live backend, and it reviews nothing
about the control-event construction; both stay open under Groups in
`implementation-checklist.md`. It leaves `AppEnvironment.beta` and its configuration, which
no entry point uses any more, to a decision of their own. The deleted MLS profile that older
records here link to is `docs/mls-profile.md` as it stood before commit `4525d3f`, and the
on-device run records are in `docs/archive/beta-mls-core/`.
...
> **Corrected by ADR-069 (2026-09-08).** The reasoning above holds and the
> outcome was right, but two wire facts in it are wrong and were wrong when written.
> `subscribe_presence` is not a frame and `auth` is not a frame: the server retired the
> first with its whole presence surface (server ADR-0022) and the second with its browser
> handshake (server ADR-0020). What was missing was therefore never a subscription this
> client could send, and the machinery left in place could never have been completed. It
> is deleted.
...
- ~~**`subscribe_presence` (D10) is unimplemented.**~~ Closed by
  ADR-069 (2026-09-08): it is not a frame, so there was nothing to implement.
  Presence is absent and that is now the finished state, not an interim one.
